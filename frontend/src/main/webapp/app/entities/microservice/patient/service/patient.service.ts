...
type RestOf<T extends IPatient | NewPatient> = Omit<T, 'dateOfBirth' | 'idIssueDate'> & {
...
export class PatientService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/patients', 'microservice');
  ...
  protected convertDateFromClient<T extends IPatient | NewPatient | PartialUpdatePatient>(patient: T): RestOf<T> {
    return {
      ...
      idIssueDate: patient.idIssueDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPatient: RestPatient): IPatient {
    return {
      ...
      idIssueDate: restPatient.idIssueDate ? dayjs(restPatient.idIssueDate) : undefined,
    };
  }
